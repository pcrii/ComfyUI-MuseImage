...
- **`dual_format`**:
  ...
  - `unified (single prompt for both)`: Produces a single unified prompt (no split delimiters) and automatically populates `prompt_g`, `prompt_l`, and `expanded_prompt` with the identical prompt. Allows you to route `prompt_g` to both encoder inputs, leave `prompt_l` unplugged, or use single-input CLIP nodes.
